...
public class FinanceFtOrgValueAddController extends BaseController
{
     ...
     * 新增资产原值增加单明细-批量
     */
    @RequiresPermissions("finance:ft:orgValueAdd:addAll")
    @Log(title = "报支类别-公司级批量新增", businessType = BusinessType.INSERT)
    @PostMapping("/batch")
    public AjaxResult addBatch(@RequestBody List<FinanceFtChange> financeFtChange)
    {
        return null;
        //return toAjax(financeFtChangeService.insertBatch(financeAoItemComps));
    }
    /**
     * 字符串是否有值
     * @param changeType 变动单类型
     * @return 字符串是否有值
     */
