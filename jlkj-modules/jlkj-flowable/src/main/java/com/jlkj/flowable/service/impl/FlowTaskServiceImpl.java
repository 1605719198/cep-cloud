package com.jlkj.flowable.service.impl;
...
import com.jlkj.flowable.common.enums.FlowComment;
import com.jlkj.flowable.domain.SysForm;
import com.jlkj.flowable.domain.dto.FlowCommentDto;
import com.jlkj.flowable.domain.dto.FlowNextDto;
import com.jlkj.flowable.domain.dto.FlowTaskDto;
import com.jlkj.flowable.domain.dto.FlowViewerDto;
import com.jlkj.flowable.domain.vo.FlowQueryVo;
import com.jlkj.flowable.domain.vo.FlowTaskVo;
import com.jlkj.flowable.factory.FlowServiceFactory;
import com.jlkj.flowable.flow.CustomProcessDiagramGenerator;
import com.jlkj.flowable.flow.FindNextNodeUtil;
import com.jlkj.flowable.flow.FlowableUtils;
import com.jlkj.flowable.service.IFlowTaskService;
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                if (StringUtils.isNotBlank(taskList.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    R<SysUser> data = remoteUserService.selectUserById(Long.parseLong(taskList.get(0).getAssignee()), SecurityConstants.INNER);
                    if (R.FAIL == data.getCode()) {
                        throw new ServiceException(data.getMsg());
                    }
                    SysUser sysUser = data.getData();
                ...
                if (StringUtils.isNotBlank(historicTaskInstance.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    R<SysUser> data = remoteUserService.selectUserById(Long.parseLong(historicTaskInstance.get(0).getAssignee()), SecurityConstants.INNER);
                    if (R.FAIL == data.getCode()) {
                        throw new ServiceException(data.getMsg());
                    }
                    SysUser sysUser = data.getData();
...
//        }
        ...
        for (Task task : taskList) {
            ...
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                    .processInstanceId(task.getProcessInstanceId())
                    .singleResult();
            R<SysUser> data = remoteUserService.selectUserById(Long.parseLong(historicProcessInstance.getStartUserId()), SecurityConstants.INNER);
            if (R.FAIL == data.getCode()) {
                throw new ServiceException(data.getMsg());
            }
            SysUser startUser = data.getData();
...
//                } else
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        R<SysUser> data = remoteUserService.selectUserById(Long.parseLong(histIns.getAssignee()), SecurityConstants.INNER);
                        if (R.FAIL == data.getCode()) {
                            throw new ServiceException(data.getMsg());
                        }
                        SysUser sysUser = data.getData();
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        ...
                        if ("candidate".equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                R<SysUser> data = remoteUserService.selectUserById(Long.parseLong(identityLink.getUserId()), SecurityConstants.INNER);
                                if (R.FAIL == data.getCode()) {
                                    throw new ServiceException(data.getMsg());
                                }
                                SysUser sysUser = data.getData();
                            ...
                            if (StringUtils.isNotBlank(identityLink.getGroupId())) {
                                R<SysRole> data = remoteRoleService.selectRoleById(Long.parseLong(identityLink.getGroupId()), SecurityConstants.INNER);
                                if (R.FAIL == data.getCode()) {
                                    throw new ServiceException(data.getMsg());
                                }
                                SysRole sysRole = data.getData();
