...
import com.jlkj.flowable.core.FormConf;
import com.jlkj.flowable.core.domain.ProcessQuery;
import com.jlkj.flowable.factory.FlowServiceFactory;
import com.jlkj.flowable.flow.FlowableUtils;
import com.jlkj.flowable.utils.ModelUtils;
import com.jlkj.flowable.utils.ProcessFormUtils;
import com.jlkj.flowable.utils.ProcessUtils;
import com.jlkj.flowable.utils.TaskUtils;
...
import com.jlkj.workflow.domain.vo.*;
import com.jlkj.workflow.mapper.WfDeployFormMapper;
import com.jlkj.workflow.service.IWfProcessService;
...
public class WfProcessServiceImpl extends FlowServiceFactory implements IWfProcessService {
     ...
     */
    @Override
    public TableDataInfoPlus<WfDefinitionVo> selectPageStartProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
...
//        definitionVoList = definitionVoList.stream().sorted(Comparator.comparing(WfDefinitionVo::getDeploymentTime).reversed()).collect(Collectors.toList());
    ...
    @Override
    public TableDataInfoPlus<WfTaskVo> selectPageOwnProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
    ...
    @Override
    public TableDataInfoPlus<WfTaskVo> selectPageTodoProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        ...
        for (Task task : taskList) {
            ...
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                ...
                .singleResult();
            R<SysUser> sysUser = userService.selectUserById(Long.parseLong(historicProcessInstance.getStartUserId()), SecurityConstants.INNER);
            SysUser startUser = sysUser.getData();
    ...
    @Override
    public TableDataInfoPlus<WfTaskVo> selectPageClaimProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        TaskQuery taskQuery = taskService.createTaskQuery()
            .active()
            .includeProcessVariables()
    ...
    @Override
    public TableDataInfoPlus<WfTaskVo> selectPageFinishedProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        Page<WfTaskVo> page = new Page<>();
        ...
        for (HistoricActivityInstance activityInstance : historicActivityInstanceList) {
            ...
            if (BpmnXMLConstants.ELEMENT_EVENT_START.equals(activityInstance.getActivityType())) {
                if (ObjectUtil.isNotNull(historicProcIns)) {
                    ...
                    R<SysUser> sysUser = userService.selectUserById(userId, SecurityConstants.INNER);
                ...
                if (StringUtils.isNotBlank(activityInstance.getAssignee())) {
                    R<SysUser> sysUser = userService.selectUserById(Long.parseLong(activityInstance.getAssignee()), SecurityConstants.INNER);
                    SysUser user = sysUser.getData();
                    elementVo.setAssigneeId(user.getUserId());
                    elementVo.setAssigneeName(user.getNickName());
                }
