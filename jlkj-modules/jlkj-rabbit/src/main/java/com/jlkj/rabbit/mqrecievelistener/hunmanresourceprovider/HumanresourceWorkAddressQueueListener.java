...
/**
 * @author Xin
 * @Description 人事模块队列监听---岗位工作地点
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourceWorkAddressQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_WORK_ADDRESS_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_WORK_ADDRESS_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_WORK_ADDRESS_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_WORK_ADDRESS_QUEUE, msg);
        }
