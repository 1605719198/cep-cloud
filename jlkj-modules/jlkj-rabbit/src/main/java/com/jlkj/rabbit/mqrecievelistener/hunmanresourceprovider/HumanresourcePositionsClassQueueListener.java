...
/**
 * @author Xin
 * @Description 人事模块队列监听---职级资料
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourcePositionsClassQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_POSITIONS_CLASS_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_POSITIONS_CLASS_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_POSITIONS_CLASS_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_POSITIONS_CLASS_QUEUE, msg);
        }
    ...
    private void executeOperation(JSONArray dataArr,String actionCode) throws Exception {
        ...
        if (actionCode.equals(HUMANRESOURCE_A)){
            ...
            dataArr.forEach( item -> {
                ...
                HumanresourcePositionsClassDTO humanresourcePositionsClassDTO = objectMapper.convertValue(item, HumanresourcePositionsClassDTO.class);
                humanResourceFeignService.addPositionsClassData(humanresourcePositionsClassDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_S)) {
            //修改
            dataArr.forEach( item -> {
                HumanresourcePositionsClassDTO humanresourcePositionsClassDTO = objectMapper.convertValue(item, HumanresourcePositionsClassDTO.class);
                humanResourceFeignService.updatePositionsClassData(humanresourcePositionsClassDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_D)) {
            //删除
            dataArr.forEach( item -> {
                HumanresourcePositionsClassDTO humanresourcePositionsClassDTO = objectMapper.convertValue(item, HumanresourcePositionsClassDTO.class);
                humanResourceFeignService.deletePositionsClassData(humanresourcePositionsClassDTO,TOKEN);
            });
        } else {
