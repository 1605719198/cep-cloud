...
import com.jlkj.common.dto.sales.SalesOrderDTO;
...
@Component
@Slf4j
public class SalesOrderQueueListener {
     ...
     *  dataSource：erp
     */
    @RabbitListener(queuesToDeclare = @Queue(SALES_SO02_QUEUE))
    public void salesContractListener(String msg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            if (dataSource.equals(SALES_DATASOURCE)) {
                //执行增删改操作
                salesOrderExecuteOperation(dataArr, actionCode);
            }
        } catch (Exception e) {
            sysQueueDlxService.saveQueueException(SALES_SO02_QUEUE, msg, e.toString());
            log.error("error:rabbitmq:{} => {}", SALES_SO02_QUEUE, msg);
        }
    }
     ...
     * @throws Exception
     */
    private void salesOrderExecuteOperation(JSONArray dataArr, String actionCode) throws Exception  {
        ...
        if (actionCode.equals(SALES_N)){
            ...
            dataArr.forEach( item -> {
                //将item复制给SalesOrderDTO
                SalesOrderDTO salesOrderDTO = objectMapper.convertValue(item, SalesOrderDTO.class);
                R result = salesFeignService.addSalesOrder(salesOrderDTO,token);
            });
        } else if (actionCode.equals(SALES_R)) {
            //修改
            dataArr.forEach( item -> {
                SalesOrderDTO salesOrderDTO = objectMapper.convertValue(item, SalesOrderDTO.class);
                R result = salesFeignService.updateSalesOrder(salesOrderDTO,token);
            });
        } else {
