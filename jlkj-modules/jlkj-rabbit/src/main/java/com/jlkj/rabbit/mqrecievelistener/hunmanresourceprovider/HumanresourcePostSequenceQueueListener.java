...
/**
 * @author Xin
 * @Description 人事模块队列监听---岗位序列资料
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourcePostSequenceQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_POST_SEQUENCE_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_POST_SEQUENCE_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_POST_SEQUENCE_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_POST_SEQUENCE_QUEUE, msg);
        }
