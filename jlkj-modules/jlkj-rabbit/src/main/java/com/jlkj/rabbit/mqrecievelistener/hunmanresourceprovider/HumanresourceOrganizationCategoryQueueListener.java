...
/**
 * @author Xin
 * @Description 人事模块队列监听---机构细类资料
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourceOrganizationCategoryQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_ORGANIZATION_CATEGORY_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_ORGANIZATION_CATEGORY_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_ORGANIZATION_CATEGORY_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_ORGANIZATION_CATEGORY_QUEUE, msg);
        }
