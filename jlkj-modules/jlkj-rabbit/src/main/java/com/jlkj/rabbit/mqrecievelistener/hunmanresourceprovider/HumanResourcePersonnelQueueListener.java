...
/**
 * @author Xin
 * @Description 人事模块队列监听--人员资料
 * @create 2022-04-28 13:59
 */
@Component
@Slf4j
public class HumanResourcePersonnelQueueListener {
    @Autowired
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_PERSONNEL_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_PERSONNEL_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_PERSONNEL_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_PERSONNEL_QUEUE, msg);
        }
    ...
    private void executeOperation(JSONArray dataArr,String actionCode) throws Exception {
        ...
        if (actionCode.equals(HUMANRESOURCE_A)){
            ...
            dataArr.forEach( item -> {
                ...
                HumanresourcePersonnelDTO humanresourcePersonnelDTO = objectMapper.convertValue(item, HumanresourcePersonnelDTO.class);
                humanResourceFeignService.addPersonnelData(humanresourcePersonnelDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_S)) {
            //修改
            dataArr.forEach( item -> {
                HumanresourcePersonnelDTO humanresourcePersonnelDTO = objectMapper.convertValue(item, HumanresourcePersonnelDTO.class);
                humanResourceFeignService.updatePersonnelData(humanresourcePersonnelDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_D)) {
            //删除
            dataArr.forEach( item -> {
                HumanresourcePersonnelDTO humanresourcePersonnelDTO = objectMapper.convertValue(item, HumanresourcePersonnelDTO.class);
                humanResourceFeignService.deletePersonnelData(humanresourcePersonnelDTO,TOKEN);
            });
        } else {
