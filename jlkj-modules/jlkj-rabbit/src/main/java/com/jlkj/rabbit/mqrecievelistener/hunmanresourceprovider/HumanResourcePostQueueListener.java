...
/**
 * @author Xin
 * @Description 人事模块队列监听--岗位资料
 * @create 2022-04-28 13:59
 */
@Component
@Slf4j
public class HumanResourcePostQueueListener {
    @Autowired
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_POST_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_POST_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_POST_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_POST_QUEUE, msg);
        }
    }
