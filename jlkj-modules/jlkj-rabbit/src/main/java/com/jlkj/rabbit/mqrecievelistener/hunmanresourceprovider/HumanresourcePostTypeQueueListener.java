...
/**
 * @author Xin
 * @Description 人事模块队列监听---岗位类别资料
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourcePostTypeQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_POST_TYPE_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_POST_TYPE_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_POST_TYPE_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_POST_TYPE_QUEUE, msg);
        }
    ...
    private void executeOperation(JSONArray dataArr,String actionCode) throws Exception {
        ...
        if (actionCode.equals(HUMANRESOURCE_A)){
            ...
            dataArr.forEach( item -> {
                ...
                HumanresourcePostTypeDTO humanresourcePostTypeDTO = objectMapper.convertValue(item, HumanresourcePostTypeDTO.class);
                humanResourceFeignService.addPostTypeData(humanresourcePostTypeDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_S)) {
            //修改
            dataArr.forEach( item -> {
                HumanresourcePostTypeDTO humanresourcePostTypeDTO = objectMapper.convertValue(item, HumanresourcePostTypeDTO.class);
                humanResourceFeignService.updatePostTypeData(humanresourcePostTypeDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_D)) {
            //删除
            dataArr.forEach( item -> {
                HumanresourcePostTypeDTO humanresourcePostTypeDTO = objectMapper.convertValue(item, HumanresourcePostTypeDTO.class);
                humanResourceFeignService.deletePostTypeData(humanresourcePostTypeDTO,TOKEN);
            });
        } else {
