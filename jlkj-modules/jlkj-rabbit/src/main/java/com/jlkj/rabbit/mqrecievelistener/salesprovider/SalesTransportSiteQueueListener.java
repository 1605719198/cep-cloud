...
/**
 * @author xk
 * @Description 汽运交运地队列监听
 * @date 2022年06月07日 16:15
 */
@Component
@Slf4j
public class SalesTransportSiteQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(SALES_SHJCBX1201_QUEUE))
    public void salesTransportEndAreaListener(String msg, Channel channel, Message message) throws Exception {
        ...
        try {
            ...
            if (dataSource.equals(SALES_DATASOURCE)) {
                //执行增删改操作
                salesTransportSiteExecuteOperation(dataArr, actionCode);
            }
        } catch (Exception e) {
            sysQueueDlxService.saveQueueException(SALES_SHJCBX1201_QUEUE, msg, e.toString());
            log.error("error:rabbitmq:{} => {}", SALES_SHJCBX1201_QUEUE, msg);
        }
    }
     ...
     * @throws Exception
     */
    private void salesTransportSiteExecuteOperation(JSONArray dataArr, String actionCode) throws Exception  {
        ...
        if (actionCode.equals(SALES_N)){
            ...
            dataArr.forEach( item -> {
                ...
                R result = salesFeignService.addSalesTransportSite(salesTransportSiteDTO,token);
            });
            ...
            dataArr.forEach( item -> {
                ...
                R result = salesFeignService.deleteSalesTransportSite(new String[]{id},token);
            });
        } else if (actionCode.equals(SALES_R)) {
            //修改
            dataArr.forEach( item -> {
                SalesTransportSiteDTO salesTransportSiteDTO = objectMapper.convertValue(item, SalesTransportSiteDTO.class);
                R result = salesFeignService.updateSalesTransportSite(salesTransportSiteDTO,token);
            });
        } else{
