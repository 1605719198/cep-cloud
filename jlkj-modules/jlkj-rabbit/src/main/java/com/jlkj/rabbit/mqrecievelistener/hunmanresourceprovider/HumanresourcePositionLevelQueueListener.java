...
/**
 * @author Xin
 * @Description 人事模块队列监听---职位等级资料
 * @create 2022-05-19 09:18
 */
@Component
@Slf4j
public class HumanresourcePositionLevelQueueListener {
     ...
     * @param message
     */
    @RabbitListener(queuesToDeclare = @Queue(HUMANRESOURCE_POSITIONS_LEVEL_QUEUE))
    public void receiveListener(String msg, Channel channel, Message message) throws Exception {
        log.info("rabbitmq:{} => {}", HUMANRESOURCE_POSITIONS_LEVEL_QUEUE, msg);
        try {
            //接到msg转换为json
            ...
            sysQueueDlxService.saveQueueException(HUMANRESOURCE_POSITIONS_LEVEL_QUEUE, msg, e.toString());
            log.error("error:rabbitmq{} => {}", HUMANRESOURCE_POSITIONS_LEVEL_QUEUE, msg);
        }
    ...
    private void executeOperation(JSONArray dataArr,String actionCode) throws Exception {
        ...
        if (actionCode.equals(HUMANRESOURCE_A)){
            ...
            dataArr.forEach( item -> {
                ...
                HumanresourcePositionLevelDTO humanresourcePositionLevelDTO = objectMapper.convertValue(item, HumanresourcePositionLevelDTO.class);
                humanResourceFeignService.addPositionLevelData(humanresourcePositionLevelDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_S)) {
            //修改
            dataArr.forEach( item -> {
                HumanresourcePositionLevelDTO humanresourcePositionLevelDTO = objectMapper.convertValue(item, HumanresourcePositionLevelDTO.class);
                humanResourceFeignService.updatePositionLevelData(humanresourcePositionLevelDTO,TOKEN);
            });
        } else if (actionCode.equals(HUMANRESOURCE_D)) {
            //删除
            dataArr.forEach( item -> {
                HumanresourcePositionLevelDTO humanresourcePositionLevelDTO = objectMapper.convertValue(item, HumanresourcePositionLevelDTO.class);
                humanResourceFeignService.deletePositionLevelData(humanresourcePositionLevelDTO,TOKEN);
            });
        } else {
