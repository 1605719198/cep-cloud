<template>
  <div class="app-container">
    <el-form :model="query" ref="query" :inline="true"  label-width="68px">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" v-hasPermi="['listMaterialsWeighingPerformance']" @click="handleQuery">搜索</el-button>
    ...
    <el-table v-loading="table.loading" :data="tableData" @sort-change="handleSort" height="67vh">
      ...
      <el-table-column label="榜单号"
                       ...
                       align="left"
                       prop="wgt_list_no" />
      <el-table-column label="毛重(t)"
                       sortable
                       minWidth="150"
                       align="left"
                       prop="gross_wgt" >
        <template scope="scoped">
          {{ (scoped.row.gross_wgt/1000).toFixed(3)}}
        </template>
      </el-table-column>
      <el-table-column label="皮重(t)"
                       sortable
                       minWidth="150"
                       align="left"
                       prop="tart_wgt" >
        <template scope="scoped">
          {{ (scoped.row.tart_wgt/1000).toFixed(3)}}
        </template>
      </el-table-column>
      <el-table-column label="净重(t)"
                       ...
                       prop="net_wgt" >
        <template scope="scoped">
          {{ (scoped.row.net_wgt/1000).toFixed(3)}}
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listMaterialsWeighingPerformance } from "@/api/material/mr/performance/performance";
export default {
  data () {
    return {
      page: {
  ...
  methods: {
    ...
    onLoad () {
      ...
      listMaterialsWeighingPerformance(this.page, this.query).then(res => {
