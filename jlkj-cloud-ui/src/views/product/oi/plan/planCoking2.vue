    ...
    <div v-if="openDialog.open">
      <el-dialog :title="openDialog.title"
        ...
        <template v-if="openDialog.type === 'add' || openDialog.type === 'edit'">
          <planCokingAdd :type="openDialog.type"
...
<script>
import {
  getProductionTargetItem,
  listYearProductionTargetPlans,
  deleteYearProductionTargetPlan,
  listMonthProductionTargetPlans,
  listDateProductionTargetPlans,
} from "@/api/production/oi/plan";
  ...
  created () {
    getProductionTargetItem({ targetItemTypeId: 1 }).then(response => {
      ...
      response.data.forEach(function (val) {
        let column = { label: val.targetItemName + '产率%', prop: val.id, minWidth: '150px' };
        let column2 = { targetItemName: val.targetItemName + '产率', id: val.id, value: "0", unit: val.measuringUnit };
        columns.push(column);
  ...
  methods: {
    ...
    onLoad () {
      ...
      if (this.queryYear !== '' && this.queryMonth === '') {
        //月计划
        listMonthProductionTargetPlans(this.page, this.query).then(res => {
        ...
        //日计划
        listDateProductionTargetPlans(this.page, this.query).then(res => {
          this.table.loading = false;
          this.tableData = res.data;//表格数据
        }, error => {
          this.table.loading = false;
        ...
        //年计划
        listYearProductionTargetPlans(this.page, this.query).then(res => {
          this.table.loading = false;
          this.tableData = res.data;//表格数据
        }, error => {
          this.table.loading = false;
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将删除' + row.plan_year + '年全部焦化指标计划, 是否继续?', '提示', {
