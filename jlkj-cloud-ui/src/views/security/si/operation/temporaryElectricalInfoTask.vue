      ...
      <el-table size="small" :data="tableData" stripe>
        ...
        <el-table-column prop="is_approval" label="审批状态" width="70">
          <template slot-scope="scope">
            {{
            scope.row.confirm_time === '' ? '未确认' : scope.row.is_approval === 0 ? '未审批' : scope.row.is_approval === 1 ?
            '已审批' : ''
            }}
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    name: "temporaryElectricalInfoTask",
    props: ['data', 'type', 'ticketId', 'level'],
    components: {vueEsign},
    data() {
      return {
        tableData: [],
    ...
    methods: {
      onLoad() {
        if (this.formId || this.ticketId) {
          req('get', 'temporaryelectricity/approval/list', {id: this.formId || this.ticketId}).then(res => {
          ...
          req('get', 'temporaryelectricity/approval/list', {id: this.data.copyId}).then(res => {
            let d = [];
          ...
          req('get', 'temporaryelectricity/approval/template/list', {}).then(res => {
            let data = res.data.data;
            let d = [];
            data.forEach(item => {
              d.push({
                ...{approval_person_id: '', approval_person_name: '', pick_person_id: '', pick_person_name: '',},
                ...item
              })
            });
            this.tableData = d;
      ...
      handleNodeClick(data) {
        ...
        if (this.tableData[this.editIndex].id) {
          req('post', 'temporaryelectricity/update/approval', {id: this.tableData[this.editIndex].id, ...this.tableData[this.editIndex]}).then(res => {
        ...
        if (this.resultImg === '') {
          ...
          setTimeout(() => {
            ...
            upload(formData).then(res => {
              ...
              req('post', 'temporaryelectricity/confirm/approval', {
                id: this.form.id,
      ...
      onConfirm(row) {
        if (row.sort === 1) {
          req('get', 'temporaryelectricity/safetymeasures/list', {id: this.formId || this.ticketId}).then(res => {
      ...
      confirmTask(row) {
        req('post', 'temporaryelectricity/confirm/approval/time', {
          id: row.id,
        }).then(() => {
      ...
      onReturn(row) {
        this.$confirm('是否确定回退审批?', '提示', {
          ...
          req('post', 'temporaryelectricity/confirm/return', data).then(() => {
    ...
    watch: {
      ...
      level(value) {
        req('get', 'temporaryelectricity/approval/template/list', {job_level: value}).then(res => {
          let data = res.data.data;
          let d = [];
          data.forEach(item => {
            d.push({
