...
<script>
  ...
  export default {
    ...
    methods: {
      onLoad() {
        if (this.formId || this.ticketId) {
          req('get', 'cutroad/approval/list', {id: this.formId || this.ticketId}).then(res => {
          ...
          req('get', 'cutroad/approval/list', {id: this.data.copyId}).then(res => {
            let d = [];
          ...
          req('get', 'cutroad/approval/template/list', {}).then(res => {
      ...
      handleNodeClick(data) {
        ...
        if (this.tableData[this.editIndex].id) {
          req('post', 'cutroad/update/approval', {id: this.tableData[this.editIndex].id, ...this.tableData[this.editIndex]}).then(res => {
        ...
        if (this.resultImg === '') {
          ...
          setTimeout(() => {
            ...
            upload(formData).then(res => {
              ...
              req('post', 'cutroad/confirm/approval', {
                id: this.form.id,
      ...
      onConfirm(row) {
        if (row.sort === 1) {
          req('get', 'cutroad/safetymeasures/list', {id: this.formId || this.ticketId}).then(res => {
      ...
      confirmTask(row) {
        req('post', 'cutroad/confirm/approval/time', {
          id: row.id,
        }).then(() => {
      ...
      onReturn(row) {
        this.$confirm('是否确定回退审批?', '提示', {
          ...
          req('post', 'cutroad/confirm/return', data).then(() => {
    ...
    watch: {
      ...
      level(value) {
        req('get', 'cutroad/approval/template/list', {job_level: value}).then(res => {
          let data = res.data.data;
          let d = [];
          data.forEach(item => {
            d.push({
