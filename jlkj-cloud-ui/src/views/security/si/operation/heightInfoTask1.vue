  ...
  <div class="avue-crud">
    <el-form :model="form" ref="form" label-position="left" label-width="80px">
      <el-table size="small" :data="tableData" stripe>
        ...
        <el-table-column prop="approval_person_name" label="审批人" width="70px"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      onLoad() {
        if (this.formId || this.ticketId) {
          req('get', 'high/approval/list', {id: this.formId || this.ticketId}).then(res => {
          ...
          req('get', 'high/approval/list', {id: this.data.copyId}).then(res => {
            let d = [];
      ...
      handleNodeClick(data) {
        ...
        if (this.tableData[this.editIndex].id) {
          req('post', 'high/update/approval', {id: this.tableData[this.editIndex].id, ...this.tableData[this.editIndex]}).then(res => {
      ...
      onConfirm(row) {
        if (row.sort === 1) {
          req('get', 'high/safetymeasures/list', {id: this.formId || this.ticketId}).then(res => {
      ...
      onReturn(row) {
        this.$confirm('是否确定回退审批?', '提示', {
          ...
          req('post', 'high/confirm/return', data).then(() => {
    ...
    watch: {
      ...
      level(value) {
        if (this.isUpdated) {
          req('get', 'high/approval/template/list', {job_level: value}).then(res => {
