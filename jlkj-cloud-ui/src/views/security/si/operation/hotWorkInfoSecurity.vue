    ...
    <el-table size="small" :data="tableData" stripe>
      <el-table-column prop="safety_measures" label="安全措施">
        <template slot-scope="scope">
          <el-input v-if="scope.row.is_confirm === 0" v-model="scope.row.safety_measures"/>
      ...
      <el-table-column label="确认" width="100px" fixed="right" v-if="data.status === 0">
        <template slot-scope="scope">
          <el-button v-if="scope.row.is_confirm === 0" size="mini" plain type="primary"
                     @click="handleConfirm(scope.row)">确定
          </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      onLoad() {
        if (this.form.id) {
          req('get', 'fire/safetymeasures/list', {id: this.form.id}).then(res => {
          ...
          req('get', 'fire/safetymeasures/list', {id: this.ticketId}).then(res => {
            this.tableData = res.data.data;//表格数据
            this.loading = false;
          }, error => {
            this.loading = false;
      ...
      handleNodeClick(data) {
        ...
        req('post', 'fire/confirm/safetymeasures', {
          id: this.row.id,
      ...
      handleCancel(id) {
        req('post', 'fire/cancel/safetymeasures', {id: id}).then(() => {
