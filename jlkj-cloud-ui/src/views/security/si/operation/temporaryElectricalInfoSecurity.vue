...
<script>
  ...
  export default {
    name: "temporaryElectricalInfoSecurity",
    ...
    methods: {
      onLoad() {
        if (this.form.id) {
          req('get', 'temporaryelectricity/safetymeasures/list', {id: this.form.id}).then(res => {
          ...
          req('get', 'temporaryelectricity/safetymeasures/list', {id: this.ticketId}).then(res => {
            this.tableData = res.data.data;//表格数据
            this.loading = false;
          }, error => {
            this.loading = false;
      ...
      // handleConfirm(row) {
      //   req('post', 'temporaryelectricity/confirm/safetymeasures', {
      //     id: row.id,
      //     safety_measures: row.safety_measures,
      //     confirm_person_id: this.$store.getters.userInfo.userId,
      //     confirm_person_name: this.$store.getters.userInfo.nickName,
      //   }).then(() => {
      ...
      handleCancel(id) {
        req('post', 'temporaryelectricity/cancel/safetymeasures', {id: id}).then(() => {
