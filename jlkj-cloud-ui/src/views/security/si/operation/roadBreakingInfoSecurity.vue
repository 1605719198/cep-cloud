...
<script>
  ...
  export default {
    ...
    methods: {
      onLoad() {
        if (this.form.id) {
          req('get', 'cutroad/safetymeasures/list', {id: this.form.id}).then(res => {
          ...
          req('get', 'cutroad/safetymeasures/list', {id: this.ticketId}).then(res => {
            this.tableData = res.data.data;//表格数据
            this.loading = false;
          }, error => {
            this.loading = false;
      ...
      // handleConfirm(row) {
      //   req('post', 'cutroad/confirm/safetymeasures', {
      //     id: row.id,
      ...
      handleCancel(id) {
        req('post', 'cutroad/cancel/safetymeasures', {id: id}).then(() => {
