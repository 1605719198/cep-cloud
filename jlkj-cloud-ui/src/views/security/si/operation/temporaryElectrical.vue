<template>
  <div class="avue-crud el-card__body" style="background-color: #FFF;width:99%">
    <div class="avue-crud__search">
      <el-form :inline="true">
            <el-form-item label="作业日期" prop="planDate">
              <el-date-picker v-model="planDate" type="daterange" start-placeholder="计划开始日期" range-separator="-"
                              end-placeholder="计划结束日期"/>
            </el-form-item>
            <el-form-item label="状态" prop="status">
              <el-select v-model="query.status" placeholder="请选择状态" :popper-append-to-body="false" clearable>
            ...
            <div class="el-form-item__content" style="margin-left: 0px;">
              <el-button v-hasPermi="['temporaryelectricity_list']" size="mini" icon="el-icon-search" type="primary"
      ...
      <div class="el-form-item__content">
        <el-button v-hasPermi="['temporaryelectricity_list']" type="primary" size="mini" icon="el-icon-plus"
                   style="margin: 5px 0;"
                   plain
      ...
      <el-form>
        <el-table height="70vh" size="small" v-loading="table.loading" :data="tableData" stripe @sort-change="handleSort">
          ...
          <el-table-column fixed="right" label="操作" width="380px">
            <template slot-scope="scope">
              <el-button v-hasPermi="['temporaryelectricity_list']" size="mini" plain icon="el-icon-edit" type="primary"
                         @click="handleOpenWindow('edit',scope.$index, scope.row)"> 查看/确定
              </el-button>
              <el-button v-hasPermi="['temporaryelectricity_list']" size="mini" plain icon="el-icon-delete" type="text"
                         @click="handleDelete(scope.$index, scope.row)" :disabled="scope.row.status !== 0">删除
              </el-button>
              <el-button v-hasPermi="['temporaryelectricity_list']" size="mini" plain icon="el-icon-document-copy" type="primary"
                         @click="handleOpenWindow('copy',scope.$index, scope.row)">复制
              </el-button>
              <el-button v-hasPermi="['temporaryelectricity_list']" size="mini" plain icon="el-icon-printer" type="info"
    ...
    <div v-if="openDialog.open">
      <el-dialog :title="openDialog.title"
        ...
        <template v-if="openDialog.type === 'add' || openDialog.type === 'edit'">
          <temporaryElectricalInfo :type="openDialog.type"
...
<script>
  ...
  import {getSafetyArea, tempElectricityInfoApi} from "@/api/security/si/WorkTicketApi";
  import temporaryElectricalInfo from "./temporaryElectricalInfo";

  export default {
    components: {
      temporaryElectricalInfo,
    },
    name: "temporaryElectrical",
    data() {
      return {
        ...
        columns: [
          ...
          {label: '作业证编号', prop: "job_code", sortable: true, minWidth: '140px'},
          {
            label: '审批状态', prop: "status_text", sortable: true, minWidth: '100px', formatter(row) {
              return row.status === 0 ? '资料建立' : row.status === 1 ? '审批中' : row.status === 2 ? '作业中' : row.status === 3 ? '作业完成' : '';
            }
          },
          {label: '作业证状态', prop: "is_timeout", sortable: true, minWidth: '110px'},
          // {label: '作业证级别', prop: "job_level", sortable: true, minWidth: '100px'},
          {label: '作业开始时间', prop: "start_time", sortable: true, minWidth: '150px'},
          {label: '作业结束时间', prop: "end_time", sortable: true, minWidth: '150px'},
          {label: '作业区域', prop: "safe_area_name", sortable: true, minWidth: '130px'},
          {label: '作业地点', prop: "place", sortable: true, minWidth: '150px'},
          {label: '申请单位', prop: "apply_depart_name", sortable: true, minWidth: '120px'},
    ...
    mounted() {
      ...
      if (toDoId !== undefined && toDoId !== null && toDoId !== ''){
        tempElectricityInfoApi(toDoId).then(res => {
          if (res.data.code === "0"){
    ...
    methods: {
      ...
      onLoad() {
        ...
        req('get', 'temporaryelectricity/list', data).then(res => {
      ...
      handleEmpty() {
        this.planDate = [];
        this.query = {start_time: '', end_time: '', status: '', safe_area_id: '', apply_person_id: '', apply_person_name: ''};
        this.onLoad();
      },
      // 打开窗口
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          req('post', 'temporaryelectricity/delete', {id: row.id}).then((res) => {
            if (res.data.code === "0") {
              this.$message({
      ...
      handlePrint(row) {
        ...
        link.href = this.GLOBAL.pdfFileUrl + '/ureport/preview?_u=mysql:临时用电作业许可证.ureport.xml&id=' + row.id;
        link.click();
      },
      //提供给子类调用的方法
      submitSave() {
