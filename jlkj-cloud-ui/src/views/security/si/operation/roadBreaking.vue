    ...
    <div class="avue-crud__search">
      <el-form :inline="true">
            ...
            <div class="el-form-item__content" style="margin-left: 0px;">
              <el-button v-hasPermi="['cutroad_list']" size="mini" icon="el-icon-search" type="primary"
      ...
      <div class="el-form-item__content">
        <el-button v-hasPermi="['cutroad_list']" type="primary" size="mini" icon="el-icon-plus"
                   style="margin: 5px 0;"
                   plain
      ...
      <el-form>
        <el-table height="70vh" size="small" v-loading="table.loading" :data="tableData" stripe @sort-change="handleSort">
          ...
          <el-table-column fixed="right" label="操作" width="380px">
            <template slot-scope="scope">
              <el-button v-hasPermi="['cutroad_list']" size="mini" plain icon="el-icon-edit" type="primary"
                         @click="handleOpenWindow('edit',scope.$index, scope.row)"> 查看/确定
              </el-button>
              <el-button v-hasPermi="['cutroad_list']" size="mini" plain icon="el-icon-delete" type="text"
                         @click="handleDelete(scope.$index, scope.row)" :disabled="scope.row.status !== 0">删除
              </el-button>
              <el-button v-hasPermi="['cutroad_list']" size="mini" plain icon="el-icon-document-copy" type="primary"
                         @click="handleOpenWindow('copy',scope.$index, scope.row)">复制
              </el-button>
              <el-button v-hasPermi="['cutroad_list']" size="mini" plain icon="el-icon-printer" type="info"
        ...
        <div style="margin-top: 10px;padding: 25px 0px 20px;float: right" class="avue-crud__pagination">
    ...
    <div v-if="openDialog.open">
      <el-dialog :title="openDialog.title"
        ...
        <template v-if="openDialog.type === 'add' || openDialog.type === 'edit'">
          <roadBreakingInfo :type="openDialog.type"
...
<script>
  ...
  import {getSafetyArea, hotWorkInfoApi, roadBreakingInfoApi} from "@/api/security/si/WorkTicketApi";
  import roadBreakingInfo from "./RoadBreakingInfo";
  ...
  export default {
    ...
    mounted() {
      ...
      if (toDoId !== undefined && toDoId !== null && toDoId !== ''){
        roadBreakingInfoApi(toDoId).then(res => {
          if (res.data.code === "0"){
    ...
    methods: {
      ...
      onLoad() {
        ...
        req('get', 'cutroad/list', data).then(res => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          req('post', 'cutroad/delete', {id: row.id}).then((res) => {
            if (res.data.code === "0") {
              this.$message({
      ...
      handlePrint(row) {
        ...
        link.target = '_blank';
        // link.href = this.GLOBAL.pdfFileUrl + '/safe/job/cutroad/file/getPdf?id=' + row.id;
        link.href = this.GLOBAL.pdfFileUrl + '/ureport/preview?_u=mysql:断路作业许可证.ureport.xml.ureport.xml&id=' + row.id;
        link.click();
      },
      //提供给子类调用的方法
      submitSave() {
