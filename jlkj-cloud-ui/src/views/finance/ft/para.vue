  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="参数键名" prop="parameterKey">
          <el-input v-model="form.parameterKey" placeholder="请输入参数键名" class="input"/>
        </el-form-item>
        <el-form-item label="参数名称" prop="parameterDesc">
          <el-input v-model="form.parameterDesc" placeholder="请输入参数名称" class="input"/>
        </el-form-item>
        <el-form-item label="参数键值" prop="parameterValue">
          <el-input v-model="form.parameterValue" placeholder="请输入参数键值" class="input"/>
        </el-form-item>
      </el-form>
...
<script>
import { listPara, getPara, delPara, addPara, updatePara } from "@/api/finance/ft/para";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除固定资产参数维护编号为"' + ids + '"的数据项？').then(function() {
        return delPara(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('finance/para/export', {
        ...this.queryParams
