    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['finance:ft:orgValueAdd:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['finance:ft:orgValueAdd:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['finance:ft:orgValueAdd:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['finance:ft:orgValueAdd:export']"
        >导出</el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listOrgValueAdd(this.queryParams).then(response => {
        this.orgValueAddList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.uuid != null) {
            ...
            addOrgValueAdd(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('change/orgValueAdd/export', {
        ...this.queryParams
      }, `orgValueAdd_${new Date().getTime()}.xlsx`)
    },
    /** 表头标记红星*/
    addRedstar(h, {column }){
      return[
