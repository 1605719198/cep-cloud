<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="122px">
      <el-form-item label="公司" prop="cashFlowName">
...
<script>
import { listItemCompid, getItem, delItemCompid, addItemCompid, updateItemCompid } from
    "@/api/finance/aa/itemCompid";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listItemCompid(this.queryParams).then(response => {
    ...
    handleAdd() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          addItemCompid(this.form).then(response => {
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      updateItemCompid(row).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateItemCompid(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
