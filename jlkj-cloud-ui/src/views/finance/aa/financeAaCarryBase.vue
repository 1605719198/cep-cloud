  ...
  <div class="app-container">
    <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleAdd"
              v-hasPermi="['finance:financeAaCarryBase:add']"
            >新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate"
              v-hasPermi="['finance:financeAaCarryBase:edit']"
            >修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['finance:financeAaCarryBase:remove']"
            >删除</el-button>
          </el-col>
          <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>

        <el-table v-loading="loading" :data="financeAaCarryBaseList" @selection-change="handleSelectionChange">
      ...
      <el-tab-pane label="结转规则" name="second">
        <finance-aa-carry-rules ref="financeAaCarryRules" ></finance-aa-carry-rules>
      </el-tab-pane>
    </el-tabs>
...
<script>
import { listFinanceAaCarryBase, getFinanceAaCarryBase, delFinanceAaCarryBase, addFinanceAaCarryBase, updateFinanceAaCarryBase } from "@/api/finance/aa/financeAaCarryBase";
import { selectVoucherTypeList } from "@/api/finance/aa/voucherType";
import financeAaCarryRules from "@/views/finance/aa/financeAaCarryRules";
export default {
  name: "FinanceAaCarryBase",
  dicts: ['aa_voucher_status'],
  components: {financeAaCarryRules},
  data() {
    return {
  ...
  methods: {
    ...
    handleCarryRules(row) {
      ...
      this.$nextTick(() => {
        this.$refs.financeAaCarryRules.init(obj)
      })
    },
    getVoucherTypeList() {
    ...
    getList() {
      this.loading = true;
      listFinanceAaCarryBase(this.queryParams).then(response => {
        this.financeAaCarryBaseList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFinanceAaCarryBase(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFinanceAaCarryBase(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFinanceAaCarryBase(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认要删除吗？').then(function() {
        return delFinanceAaCarryBase(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('finance/financeAaCarryBase/export', {
        ...this.queryParams
      }, `financeAaCarryBase_${new Date().getTime()}.xlsx`)
    },
    /** 核算项目转中文 */
