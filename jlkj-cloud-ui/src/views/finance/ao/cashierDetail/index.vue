    ...
    <el-row :gutter="20">
      <el-col :span="6" >
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
          ...
          <el-form-item label="申请人" prop="cashierName">
            <el-input
            ...
            >
              <el-button type="primary" plain  @click="fromButton(item)">
                <el-row>
                  ...
                  <el-col :span="24" style="height: 30px">
                    <el-form-item label="申请人:" prop="cashierName">
                      <span style="text-align: center;display:block;width: 121px">{{ item.cashierName }}</span>
                    </el-form-item>
                  </el-col>
                </el-row>
              </el-button>
        ...
        <div v-if="borrowingIf">
          ...
          <el-form ref="borrowingForm" :model="borrowingForm" :rules="rules" label-width="118px">
            <el-row>
              ...
              <el-col :span="6"style="height: 30px">
                <el-form-item label="签核会计" prop="cashierName">
                  <span style="text-align: center;display:block;width: 121px">{{ borrowingForm.cashierName }}</span>
                </el-form-item>
              </el-col>

              <el-col :span="6"style="height: 30px">
                <el-form-item label="抛账日期" prop="payDate">
              ...
              <el-col :span="6"style="height: 30px">
                <el-form-item label="传票编号" prop="voucherNo" :show-overflow-tooltip='true'>
                  <span style="text-align: center;display:block;width: 121px">{{ borrowingForm.voucherNo }}</span>
                </el-form-item>
              </el-col>
              <el-col :span="6"style="height: 30px">
...
<!--                <template slot-scope="scope">
          ...
          <el-table v-loading="loading"
                    ...
                    :key="tableKey2"
                    :row-class-name="rowDetailAccountIndexAccount">
        ...
        <div  v-if="reimbursementIf">
          ...
          <el-form ref="form" :model="reimbursementForm" :rules="rules" label-width="118px">
            <el-row>
              ...
              <el-col :span="6"style="height: 30px">
                <el-form-item label="签核会计" prop="cashierName">
                  <span style="text-align: center;display:block;width: 121px">{{ reimbursementForm.cashierName }}</span>
                </el-form-item>
              </el-col>
              <el-col :span="6"style="height: 30px">
                <el-form-item label="抛账日期" prop="payDate">
                  <span style="text-align: center;display:block;width: 121px">{{ parseTime(reimbursementForm.payDate, '{y}-{m}-{d}') }}</span>
                </el-form-item>
              </el-col>
              <el-col :span="6"style="height: 30px">
                <el-form-item label="会计确认日" prop="accountDate">
        ...
        <div  v-if="repaymentIf">
          ...
          <el-form ref="repaymentForm" :model="repaymentForm" :rules="rules" label-width="118px">
            <el-row>
              ...
              <el-col :span="6"style="height: 30px">
                <el-form-item label="还款单状态" prop="status">
                  <dict-tag :options="dict.type.ao_status" :value="repaymentForm.status"/>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
...
import {
  addDetailCashier,
  getLoanADetailCashier,
  listCashierDetail,
  updateDetailCashier
} from "@/api/finance/ao/cashierDetail";
...
import {addCashier, listCashier, updateCashier, updateCashierRepayment} from "@/api/finance/ao/cashier";
...
export default {
  ...
  methods: {
    ...
    rowDetailAccountIndexAccount({row, rowIndex}) {
      row.indexAccount = rowIndex + 1;
    },
    /** 借支会计科目点击事件 */
    inputAcctName(val) {
    ...
    getAcctCodeCorpPop(val){
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'acctCode', val[0].acctCode);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'acctId', val[0].acctId);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'acctName', val[0].acctName);
    ...
    getCalTypePOP(val){
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calCodea', val.calNo);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calIda', val.Id);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calNamea', val.calName);
    },
    /**借支项目2点击事件 */
    inputCalNameb(val) {
      this.indexAccount= val.indexAccount
    ...
    getCalTypePOP2(val){
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calCodeb', val.calNo);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calIdb', val.Id);
      this.$set(this.detailAccountLoanList[this.indexAccount-1], 'calNameb', val.calName);
    },
    /** 报销项目1点击事件 */
    ...
    getList() {
      ...
      this.repaymentIf=false;
      this.loading = true;
      listCashier(this.queryParams).then(response => {
    ...
    getListLoan(val){
      getLoanApplyFrom(val).then(response => {
        ...
        listCashierDetail( this.borrowingForm).then(response => {
          if ( response.rows.length == 0){
            this.detailAccountLoanList=[]
            let itemAccount= {
              indexAccount:null,
              id: null,
              companyId: null,
    ...
    getReimbursement(val){
      getReimbursementFrom(val.billId).then(response => {
        ...
        getFormLoan( this.reimbursementForm).then(response => {
          ...
          listCashierDetail( this.reimbursementForm).then(response => {
            if ( response.rows.length == 0){
    ...
    /** 借支插行 */
    handleAddAccountLoanList(row) {
      let itemAccount= {
        indexAccount: row.indexAccount + 1,
        id: null,
        companyId: null,
        accountId: null,
        billNo: null,
      ...
      this.detailAccountLoanList.splice(row.indexAccount, 0, itemAccount)
    },
    /** 借支删除 */
    handleDeleteAccountLoanList(row) {
      if (row.indexAccount-1>0){
        this.detailAccountLoanList.splice(row.indexAccount-1,1);     //splice删除方法,1代表删除几个
      }else {
      ...
      this.$refs["borrowingForm"].validate(valid => {
        if (valid) {
          ...
          if (this.borrowingForm.detailAccountLoanList[0].id != null) {
            ...
            addDetailCashier(this.borrowingForm).then(response => {
    ...
    handleBorrowingConfirm() {
      if(this.borrowingForm.status=='10'){
        this.borrowingForm.payDate = new Date()
        this.borrowingForm.status='20'
        this.valString ="付款并抛账成功"
        this.handleBorrowingAdd()
      }else {
        this.$message.error('当前状态无法付款并抛账！');
        return
      }
    },
    /** 借支取消抛账 */
    handleCancelConfirmation() {
      this.$message.error('当前功能待开发！');
 /*     if(this.borrowingForm.status=='20'){
    ...
    handleReimbursementAdd() {
      this.$refs["reimbursementForm"].validate(valid => {
        if (valid) {
          this.reimbursementForm.financeAoAccountCashierList = this.detailAccountList;
          if (this.reimbursementForm.financeAoAccountCashierList[0].id != null) {
            updateCashier(this.reimbursementForm).then(response => {
            ...
            addCashier(this.reimbursementForm).then(response => {
              this.$modal.msgSuccess("新增成功");
              this.open = false;
              this.getList();
              this.buttonIf= true
    ...
    handleReimbursementConfirm() {
      if(this.reimbursementForm.status=='10'){
        this.reimbursementForm.payDate = new Date()
        this.reimbursementForm.status='40'
        this.ReimbursementVal = '付款并抛账成功'
    ...
    handleRepaymentAdd() {
      this.$refs["repaymentForm"].validate(valid => {
        if (valid) {
          this.repaymentForm.financeAoAccountCashierList = this.detailRepaymentAccountList;
          if (this.repaymentForm.financeAoAccountCashierList[0].id != null) {
            updateCashierRepayment(this.repaymentForm).then(response => {
            ...
            addCashier(this.repaymentForm).then(response => {
              this.$modal.msgSuccess("新增成功");
              this.open = false;
              this.getList();
              this.buttonIf= true
    ...
    handleRepaymentConfirm() {
      if(this.repaymentForm.status=='00'){
