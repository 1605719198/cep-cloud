<template>
  <div style="padding-left: 15px">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" style="padding-top: 20px" label-width="68px">
      ...
      <el-form-item label="收款方" prop="saleName">
        <el-input
    ...
    <el-tabs v-model="activeName" @tab-click="handleClick" style="margin-top:-15px" >
      <el-tab-pane label="未报销" name="first">
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleAdd"
              v-hasPermi="['ao:expenseBills:add']"
            >新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate"
              v-hasPermi="['ao:expenseBills:edit']"
            >修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['ao:expenseBills:remove']"
            >删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleExport"
              v-hasPermi="['ao:expenseBills:export']"
            >导出</el-button>
          </el-col>
      ...
      <el-tab-pane label="已报销" name="second" >
        <expenses-list ref="expensesList"></expenses-list>
      </el-tab-pane>
    </el-tabs>
...
<script>
import { listExpenseBills, getExpenseBills, delExpenseBills, addExpenseBills, updateExpenseBills } from "@/api/finance/ao/expenseBills";
import { selectCompanyList } from "@/api/finance/aa/companyGroup";
import  selectBills  from "@/views/finance/ao/expenseBills/selectBills";
import  ExpensesList from "@/views/finance/ao/expenseBills/ExpensesList";
export default {
  name: "ExpenseBills",
  dicts: ['ao_bill_type','ao_bill_medium','ao_plus_minus'],
  components: { selectBills,ExpensesList },
  data() {
    // 额度验证，保存小数点后两位
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getExpenseBills(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateExpenseBills(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addExpenseBills(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('finance/expenseBills/export', {
        ...this.queryParams
      }, `expenseBills_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
