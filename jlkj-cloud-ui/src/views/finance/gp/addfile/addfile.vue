    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="96px">
      ...
      <el-form-item label="上传资料类型" prop="dataType">
        <el-select v-model="queryParams.dataType"  placeholder="请选择上传资料类型">
          <el-option
            v-for="dict in dict.type.finance_gp_datatypeb"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="addfileList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="上传资料类型" align="center" prop="dataType" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.finance_gp_datatypeb" :value="scope.row.dataType"/>
        </template>
      </el-table-column>
...
<script>
import { listAddfileChange, getAddfile, delAddfile, addAddfile, updateAddfile } from "@/api/finance/gp/addfile";
import {getToken} from "@/utils/auth";


export default {
  name: "Addfile",
  dicts: ['finance_gp_datatypeb'],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAddfileChange(this.queryParams).then(response => {
