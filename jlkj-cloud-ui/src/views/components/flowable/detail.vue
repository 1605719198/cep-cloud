<template>
  <div class="app-container">
    <div>
      <el-card class="box-card" shadow="hover" v-if="processed === true">
...
import Parser from '@/utilsflow/generator/parser'
...
import { getHisByProcInsId } from '@/api/workflow/instance'
export default {
  name: 'detail',
  components: {
    ProcessViewer,
  ...
  methods: {
    initData() {
      this.taskForm.procInsId = this.procData.procInsId
      this.taskForm.taskId  = this.$route.query && this.$route.query.taskId;
      this.processed = this.procData.processed && eval(this.procData.processed || false);
      // 如果流程实例id为undefined，赋值为空，避免查出多条数据
      if (this.taskForm.procInsId == undefined) {
      ...
      this.$refs["taskForm"].validate(valid => {
        if (valid) {
          ...
          this.$modal.confirm('拒绝审批单流程会终止，是否继续？').then(function() {
            ...
            const rejectData = {type: res.data,formData: this.procData.formData}
