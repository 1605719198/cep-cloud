  ...
  <el-row class="row" type="flex" justify="space-around" :gutter="10">
    <el-col :span="16" style="height: 100%">
      <div class="ranking-board-title1" style="justify-content: space-between;">
        <div class="ranking-board-title">蒸汽消耗数据统计分析</div>
      ...
      <div id="place9" style="position: relative; height: 40vh;">
        <div id="chart9" style="position:absolute; height: 35vh; width: 55vw;" />
      </div>
    </el-col>

    <el-col :span="8">
      <div class="ranking-board-title">蒸汽消耗数据统计分析</div>
      <div id="place10" style="position: relative; height: 35vh;">
...
export default {
  ...
  methods: {
    drawChart() {
      ...
      this.myChart9 = this.$echarts.init(document.getElementById("chart9"));
      // 指定图表的配置项和数据
      let option = {
    ...
    engyBottom () {
      ...
      getEngyTopAnalys(this.qry1).then(res => {
        ...
        data.forEach(item => {
          ...
          if(item.energy_code_name=='低压蒸汽'){this.planData.push(item.qty)}
          if(item.energy_code_name=='超高压蒸汽'){this.planData2.push(item.qty)}
          if(item.energy_code_name=='蒸汽回收量'){this.planData3.push(item.qty)}
